...
public class BlePeripheralBattery {
    ...
    public void startReadingBatteryLevel(@NonNull final UpdateHandler updateHandler) {
        final BluetoothGattCharacteristic batteryCharacteristic = mBlePeripheral.getCharacteristic(kBatteryCharacteristicUuid, kBatteryServiceUuid);
        ...
        // Read current value
        mBlePeripheral.readCharacteristic(batteryCharacteristic, new BlePeripheral.DataReadHandler() {
            @Override
            public void completion(int status, @Nullable byte[] data) {
                if (status == BluetoothGatt.GATT_SUCCESS) {
                    mCurrentBatteryLevel = BlePeripheralBattery.this.getBatteryLevel(batteryCharacteristic);
        ...
        mBlePeripheral.characteristicEnableNotify(batteryCharacteristic, new BlePeripheral.NotifyHandler() {
            @Override
    ...
    public void stopReadingBatteryLevel(@Nullable final BlePeripheral.CompletionHandler completionHandler) {
        ...
        mBlePeripheral.characteristicDisableNotify(batteryCharacteristic, new BlePeripheral.CompletionHandler() {
            @Override
